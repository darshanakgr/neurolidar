...
def init_depthext_datasets(hdf5_file: str, metadata_file: str, clip: bool, downsample: float = 0.0) -> tuple:
    ...
    train_df = df[df.split == "train"]
    test_df = df[df.split == "test"]
    train_dataset = DepthExtDataset(hdf5_file, train_df, clip, downsample)
    test_dataset = DepthExtDataset(hdf5_file, test_df, clip, downsample)

    return train_dataset, test_dataset


def init_keyframe_datasets(hdf5_file: str) -> tuple:
    train_dataset = KeyFrameDataset(hdf5_file, split="train")
    test_dataset = KeyFrameDataset(hdf5_file, split="test")
...
def init_keyframe_dataloaders(train_dataset: Dataset, test_dataset: Dataset, config: TrainingConfig) -> tuple:
    train_loader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, num_workers=config.num_workers, drop_last=True, prefetch_factor=2)
    test_loader = DataLoader(test_dataset, batch_size=config.batch_size, shuffle=False, num_workers=config.num_workers, drop_last=False, prefetch_factor=2)

    return train_loader, test_loader


def init_optimizer(model: nn.Module, config: TrainingConfig | KFDTrainingConfig) -> optim.Optimizer:

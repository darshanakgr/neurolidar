...
class balanced_loss_function(nn.Module):
    ...
    def forward(self, output, depth):
        ...
        loss_depth = F.smooth_l1_loss(output, depth, reduction='mean')
...
def meter_loss_fn(output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    with torch.no_grad():
        ones = torch.ones(target.size(0), 1, target.size(2), target.size(3)).float().to(target.device)

    depth_grad = sobel_edge_detector(target)
    ...
    loss_normal = torch.abs(1 - F.cosine_similarity(output_normal, depth_normal, dim=1, eps=0)).mean()

    loss_ssim = (1 - ssim(output, target, val_range=1000.0))
    
    loss_depth = F.smooth_l1_loss(output, target)
